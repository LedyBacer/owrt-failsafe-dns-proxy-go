...
function updateRuntime(status, service, logs, integration) {
	...
	if (serviceNode)
		serviceNode.textContent = service.running
			? _('Running, version %s').format(service.version || '-')
			: (service.enabled ? _('Stopped') : _('Stopped and disabled'));
	if (activeNode)
...
return view.extend({
	...
	render: function(data) {
		...
		o.validate = function(sectionId, value) {
			if (!value || !/^[^:\s]+:(A|AAAA)$/i.test(value))
					...
					E('button', {
						...
						click: ui.createHandlerFn(this, function() {
							return callDnsmasqAction('dry-run').then(function(r) {
								ui.addNotification(null, E('p', {}, r.message || _('Preflight checks passed.')), 'info');
								return refreshRuntime();
							});
						})
					}, _('Dry run')),
					...
					E('button', {
						...
						click: ui.createHandlerFn(this, function() {
							return callDnsmasqAction('enable').then(function(r) {
								ui.addNotification(null, E('p', {}, r.message || _('dnsmasq integration enabled.')), r.enabled ? 'info' : 'error');
								return refreshRuntime();
							});
						})
					...
					E('button', {
						...
						click: ui.createHandlerFn(this, function() {
							return callDnsmasqAction('disable').then(function(r) {
								ui.addNotification(null, E('p', {}, r.message || _('dnsmasq integration disabled.')), 'info');
								return refreshRuntime();
							});
						})
